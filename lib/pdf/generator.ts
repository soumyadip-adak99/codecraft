...
export async function generateSessionPDF(data: SessionReportData): Promise<Buffer> {
  ...
  const boxes = [
    { label: "Total Solved", value: String(data.totalSolved), color: [249, 115, 22] as [number,number,number] },
    { label: "Easy", value: String(data.easySolved), color: [34, 197, 94] as [number,number,number] },
    { label: "Medium", value: String(data.mediumSolved), color: [251, 191, 36] as [number,number,number] },
    { label: "Hard", value: String(data.hardSolved), color: [239, 68, 68] as [number,number,number] },
  ];
